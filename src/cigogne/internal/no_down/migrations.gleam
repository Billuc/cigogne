...
pub fn find_migration(
  ...
  migrations
  |> list.find(fn(m) { m.index == data.index && m.name == data.name })
  |> result.replace_error(types.MigrationNotFoundError(data.index, data.name))
}
...
pub fn compare_migrations(
  ...
  int.compare(migration_a.index, migration_b.index)
}
...
pub fn find_n_migrations_to_apply(
  ...
  case count {
    ...
    n if n < 0 ->
      Error(types.UnexpectedValueError("A positive integer", n |> int.to_string))
    _ -> Ok([])
  }
